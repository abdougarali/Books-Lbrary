...
export default function Home() {
  // State لإدارة Modal تفاصيل الكتاب
  const [selectedBook, setSelectedBook] = useState<Book | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);

  /**
   * دالة لفتح Modal تفاصيل الكتاب
   */
  const handleBookClick = (book: Book) => {
    setSelectedBook(book);
    setIsModalOpen(true);
    
    // تتبع حدث Google Analytics: عرض تفاصيل الكتاب
    trackGAEvent("book_details_view", {
      book_title: book.title,
      book_author: book.author,
      event_category: "engagement",
      event_label: "Book Details View",
    });
  };
  ...
  const handleCloseModal = () => {
    ...
    // تأخير بسيط قبل تنظيف selectedBook لتحسين animation
    setTimeout(() => {
          ...
          >
            {books.slice(0, 11).map((book, index) => (
              <SwiperSlide key={book.id}>
                <div 
                  className="group bg-white rounded-2xl shadow-sm overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 h-full border border-[#c6bbae]/40 cursor-pointer"
                ...
                >
                  ...
                  <div className="p-5 sm:p-6 font-cairo flex flex-col h-[calc(100%-18rem)]">
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation(); // منع فتح Modal عند النقر على الزر
                        handleWhatsAppOrder(book.title, book.author, book.price);
                      }}
